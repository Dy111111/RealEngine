...
namespace YAML {
	...
	template<>
	struct convert<Real::UUID>
	{
		static Node encode(const Real::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Real::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		{
			RE_CORE_ERROR("Failed to load .hazel file '{0}'\n     {1}", filepath, e.what());
			return false;
		}
		if (!data["Scene"])
			return false;
