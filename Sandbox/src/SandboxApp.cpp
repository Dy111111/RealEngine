...
public:
	...
	virtual void OnAttach() override
	{
		...
		m_Mesh.reset(new Real::Mesh("assets/meshes/cerberus.fbx"));
		m_SphereMesh.reset(new Real::Mesh("assets/models/Sphere.fbx"));
	...
	virtual void OnUpdate() override
	{
		...
		Real::UniformBufferDeclaration<sizeof(mat4), 1> quadShaderUB;
		...
		Real::UniformBufferDeclaration<sizeof(mat4) * 2 + sizeof(vec3) * 4 + sizeof(float) * 8, 14> simplePbrShaderUB;
...
private:
	std::unique_ptr<Real::Shader> m_Shader;
	std::unique_ptr<Real::Shader> m_PBRShader;
	std::unique_ptr<Real::Shader> m_SimplePBRShader;
	std::unique_ptr<Real::Shader> m_QuadShader;
	std::unique_ptr<Real::Shader> m_HDRShader;
	std::unique_ptr<Real::Mesh> m_Mesh;
	std::unique_ptr<Real::Mesh> m_SphereMesh;
	std::unique_ptr<Real::Texture2D> m_BRDFLUT;

	struct AlbedoInput
	{
		glm::vec3 Color = { 0.972f, 0.96f, 0.915f }; // Silver, from https://docs.unrealengine.com/en-us/Engine/Rendering/Materials/PhysicallyBased
		std::unique_ptr<Real::Texture2D> TextureMap;
		bool SRGB = true;
	...
	std::unique_ptr<Real::VertexBuffer> m_VertexBuffer;
	std::unique_ptr<Real::IndexBuffer> m_IndexBuffer;
	std::unique_ptr<Real::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;
	...
	// Editor resources
	std::unique_ptr<Real::Texture2D> m_CheckerboardTex;
};
