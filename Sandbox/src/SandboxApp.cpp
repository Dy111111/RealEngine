...
public:
	EditorLayer()
		: m_Scene(Scene::Model), m_Camera(glm::perspectiveFov(glm::radians(45.0f), 
	...
	{
		using namespace glm;
		m_SimplePBRShader.reset(Real::Shader::Create("assets/shaders/simplepbr.glsl"));
		m_QuadShader.reset(Real::Shader::Create("assets/shaders/quad.glsl"));
		m_HDRShader.reset(Real::Shader::Create("assets/shaders/hdr.glsl"));
		m_GridShader.reset(Real::Shader::Create("assets/shaders/Grid.glsl"));
		m_Mesh.reset(new Real::Mesh("assets/models/m1911/m1911.fbx"));

		m_SphereMesh.reset(new Real::Mesh("assets/models/Sphere1m.fbx"));
		m_PlaneMesh.reset(new Real::Mesh("assets/models/Plane1m.obj"));

		// Editor
		m_CheckerboardTex.reset(Real::Texture2D::Create("assets/editor/Checkerboard.tga"));

		// Environment
		m_EnvironmentCubeMap.reset(Real::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Radiance.tga"));
		m_EnvironmentIrradiance.reset(Real::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Irradiance.tga"));
		m_BRDFLUT.reset(Real::Texture2D::Create("assets/textures/BRDF_LUT.tga"));

		m_Framebuffer.reset(Real::Framebuffer::Create(1280, 720, Real::FramebufferFormat::RGBA16F));
		m_FinalPresentBuffer.reset(Real::Framebuffer::Create(1280, 720, Real::FramebufferFormat::RGBA8));

		m_PBRMaterial.reset(new Real::Material(m_SimplePBRShader));

		float x = -4.0f;
		float roughness = 0.0f;
		for (int i = 0; i < 8; i++)
		{
			Real::Ref<Real::MaterialInstance> mi(new Real::MaterialInstance(m_PBRMaterial));
		...
		m_VertexBuffer.reset(Real::VertexBuffer::Create());
		m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

		uint32_t* indices = new uint32_t[6]{ 0, 1, 2, 2, 3, 0, };
		m_IndexBuffer.reset(Real::IndexBuffer::Create());
	...
	virtual void OnUpdate(Real::TimeStep ts) override
	{
		// THINGS TO LOOK AT:
		// - BRDF LUT
	...
	virtual void OnImGuiRender() override
	{
		...
		ImGui::Separator();
		{
			...
			{
				std::string filename = Real::Application::Get().OpenFile("");
				if (filename != "")
					m_Mesh.reset(new Real::Mesh(filename));
			}
		}
		ImGui::Separator();
			...
			if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Real::Application::Get().OpenFile("");
						if (filename != "")
							m_AlbedoInput.TextureMap.reset(Real::Texture2D::Create(filename, m_AlbedoInput.SRGB));
					}
				}
				ImGui::SameLine();
				ImGui::BeginGroup();
				...
				{
					if (m_AlbedoInput.TextureMap)
						m_AlbedoInput.TextureMap.reset(Real::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
				}
				ImGui::EndGroup();
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Real::Application::Get().OpenFile("");
						if (filename != "")
							m_NormalInput.TextureMap.reset(Real::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Real::Application::Get().OpenFile("");
						if (filename != "")
							m_MetalnessInput.TextureMap.reset(Real::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Real::Application::Get().OpenFile("");
						if (filename != "")
							m_RoughnessInput.TextureMap.reset(Real::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
		...
		{
			auto& shaders = Real::Shader::s_AllShaders;
	...
	virtual void OnEvent(Real::Event& event) override
	{
	}
private:
	Real::Ref<Real::Shader> m_SimplePBRShader;
	Real::Scope<Real::Shader> m_QuadShader;
	Real::Scope<Real::Shader> m_HDRShader;
	Real::Scope<Real::Shader> m_GridShader;
	Real::Scope<Real::Mesh> m_Mesh;
	Real::Scope<Real::Mesh> m_SphereMesh, m_PlaneMesh;
	Real::Ref<Real::Texture2D> m_BRDFLUT;

	Real::Ref<Real::Material> m_PBRMaterial;
	std::vector<Real::Ref<Real::MaterialInstance>> m_MetalSphereMaterialInstances;
	std::vector<Real::Ref<Real::MaterialInstance>> m_DielectricSphereMaterialInstances;
	...
	struct AlbedoInput
	{
		...
		Real::Ref<Real::Texture2D> TextureMap;
		bool SRGB = true;
	...
	std::unique_ptr<Real::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

	Real::Ref<Real::VertexBuffer> m_VertexBuffer;
	Real::Ref<Real::IndexBuffer> m_IndexBuffer;
	Real::Ref<Real::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;

	Real::Camera m_Camera;
	...
	// Editor resources
	Real::Ref<Real::Texture2D> m_CheckerboardTex;
};
