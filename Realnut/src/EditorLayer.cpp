...
namespace Real {
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		m_SceneHierarchyPanel.OnImGuiRender();//场景层次结构
		m_ContentBrowserPanel.OnImGuiRender();//资源管理器
		...
		// Gizmos
		Entity selectedEntity = m_SceneHierarchyPanel.GetSelectedEntity();//获取选中实体
	...
	{
		if (e.GetMouseButton() == Mouse::ButtonLeft)//左键选中
		{
	...
	{
		std::string filepath = FileDialogs::OpenFile("Real Scene (*.hazel)\0*.hazel\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
